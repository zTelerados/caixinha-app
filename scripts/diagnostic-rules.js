...
const rules = {
  ...
  'Banco de Dados / Persistencia': {
    checks: [
      ...
      { id: 'no_backup_script',    weight: -5, desc: 'PENALTY: sem backup strategy',                 test: (ctx) => !ctx.fileExists('scripts/backup.js') && !ctx.fileExists('scripts/backup.sh') },
  ...
  'Dashboard Web': {
    checks: [
      ...
      { id: 'no_drill_down',       weight: -5, desc: 'PENALTY: sem drill-down em categorias',        test: (ctx) => !ctx.fileExists('src/app/category/[id]/page.tsx') },
      ...
      { id: 'csv_export',          weight: 4,  desc: 'Export CSV implementado',                      test: (ctx) => ctx.fileContains('src/app/page.tsx', 'csv') || ctx.fileContains('src/app/page.tsx', 'export') },
    ]
  },
